...
type ReviewAction = 'approve' | 'revisions' | 'reject';

export default function SupervisorProposalReviewPage() {
  ...
  const handleAddComment = async () => {
    if (!commentSection || !commentText.trim()) {
      toast.error('Select a section and enter a comment');
      return;
    }
    const latestVersion = proposal!.versions[proposal!.versions.length - 1];
    setAddingComment(true);
    try {
      await api.post(`/proposal-versions/${latestVersion.id}/comments`, {
  ...
  const handleReviewAction = async () => {
    ...
    try {
      ...
      const messages: Record<ReviewAction, string> = {
        approve: 'Proposal approved for admin review',
      ...
      setActionReason('');
      router.push('/supervisor/proposals');
    } catch (err: unknown) {
  ...
  const actionLabels: Record<ReviewAction, string> = {
    approve: 'Approve for Admin Review',
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {proposal.workspace.student.user.name} &middot; Version{' '}
            {latestVersion.versionNumber} &middot;{' '}
                ...
                <CardContent className="pt-4 space-y-3">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setCommentSection(v ?? '')}
                    >
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder="Select section" />
                      </SelectTrigger>
                      <SelectContent>
                        {PROPOSAL_SECTIONS.filter((s) => s.key !== 'title').map(
                          (s) => (
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              {confirmAction === 'approve' &&
                'This will forward the proposal to the admin for final review.'}
              {confirmAction === 'revisions' &&
                'The student will be asked to revise and resubmit their proposal.'}
